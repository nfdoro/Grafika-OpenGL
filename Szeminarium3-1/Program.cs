...
namespace GrafikaSzeminarium
{
    internal class Program
    {
        ...
        private static PanelModelDescriptor panel;
        private static PanelModelDescriptor customPanel;
        ...
        private static void GraphicWindow_Load()
        {
            ...
            {
                -MathF.Sin(normalOffset),
                0f,
                MathF.Cos(normalOffset)
            };

            // jobb oldal
            float[] normalRight = new float[]
            {
                MathF.Sin(normalOffset),
                0f,
                MathF.Cos(normalOffset)
        ...
        private static void DrawPanels()
        {
            ...
            float width = 1f;
            float height = 2f;
            float radius = (width / 2f) / MathF.Tan(MathF.PI / count);
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                var scale = Matrix4X4.CreateScale(width, height, 1f);
                var rotationY = Matrix4X4.CreateRotationY(angle);
                var translationLeft = Matrix4X4.CreateTranslation(x - offsetBetweenBarrels, 0f, z);
                var translationRight = Matrix4X4.CreateTranslation(x + offsetBetweenBarrels, 0f, z);
        ...
        private static unsafe void DrawModelObject(PanelModelDescriptor model)
        {
            ...
            Gl.BindBuffer(GLEnum.ElementArrayBuffer, model.Indices);
            Gl.DrawElements(PrimitiveType.Triangles, model.IndexArrayLength, DrawElementsType.UnsignedInt, null);
